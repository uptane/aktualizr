...
def generate_repo(repo_path, akt_repo):
    arepo = [akt_repo, '--keytype', 'dummy']
...
def run_test(akt_test, server, srcdir, pf):
...
def main():
    ...
    with tempfile.TemporaryDirectory() as repo_dir:
        generate_repo(repo_dir, args.akt_repo)

        with FakeTestServerBackground(repo_dir, srcdir=srcdir) as uptane_server:
            ...
            with multiprocessing.Pool(args.jobs) as pool:
                f = functools.partial(run_test, path.abspath(args.akt_test),
                                      server, srcdir, args.probability_failure)
                for r in pool.imap(functools.partial(pooled_test, f),
                                   range(1, args.n_tests+1)):
                    if not r:
                        print('error found!')
                        pool.close()
                        break
