...
| *SWUpdate* |  SWUpdate is a tool for managing Linux filesystems, and delivers atomic updates. SWUpdate is a traditional dual-bank systems, having an automatic backup partition that boots on failure.

You probably won't have to use the SWUpdate CLI often, but it's handy for troubleshooting build issues.
