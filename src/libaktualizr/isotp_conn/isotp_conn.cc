...
IsoTpSendRecv::IsoTpSendRecv(std::string can_iface_, uint16_t canaddr_rx_, uint16_t canaddr_tx_) : can_iface{can_iface_}, canaddr_rx{canaddr_rx_}, canaddr_tx{canaddr_tx_} {
  ...
  filter.can_id = canaddr_rx & 0x3FF;
  filter.can_mask = 0x3FF;
...
bool IsoTpSendRecv::canSend(const uint32_t arbitration_id, const uint8_t* data, const uint8_t size, void* private_data) {
...
bool IsoTpSendRecv::Send(const std::string& out) {
  IsoTpMessage message_tx =
      isotp_new_send_message(canaddr_tx, reinterpret_cast<const uint8_t*>(out.c_str()), static_cast<uint16_t>(out.length()));
...
bool IsoTpSendRecv::Recv(std::string* in) {
  IsoTpReceiveHandle recv_handle = isotp_receive(&isotp_shims, canaddr_rx, nullptr);
