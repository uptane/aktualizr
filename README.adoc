...
* Implementation of **offline-updates**: this is an initial attempt to support offline-updates as described in https://github.com/uptane/pures/blob/main/pure2.md[pure-2]; currently aktualizr must be configured to operate either in online or in offline mode, but there are plans to support both modes simultaneously in the future.
