...
bool uptane_verify_firmware_init(void) {
  ...
  if(state) {
    if(!memcmp(expected_hash->hash, state->firmware_hash.hash, (size_t) crypto_get_hashlen(alg))) {
...
void uptane_verify_firmware_feed(const uint8_t* data, size_t len) {
  crypto_hash_feed(&hash_context, data, len);
}

bool uptane_verify_firmware_finalize(void) {
  crypto_hash_t computed_hash;
  crypto_hash_result(&hash_context, &computed_hash);
  return !memcmp(computed_hash.hash, expected_hash->hash, (size_t) crypto_get_hashlen(expected_hash->alg));
}
