...
void message_received(const IsoTpMessage* message) {
	...
	switch (message->payload[0]) {
		case 0x10: /* DiagnosticSessionControl */
			...
			send_uds_positive_sessioncontrol(message->payload[1]);
			break;
		...
		case 0x31: /* RoutineControl*/
			...
			if(res)
				...
				send_uds_positive_routinecontrol(message->payload[1], (message->payload[2] << 8) | message->payload[3]);
			break;
		case 0x11: /* ECUReset */
			if(message->size < 2) {
			...
			send_uds_positive_ecureset(message->payload[1]);
			can_flush_send();
			NVIC_SystemReset();
			break;
		...
		case 0x22: /* ReadDataByIdentifier */
			...
			break;
		default:
			send_uds_error(message->payload[0], 0x11); /* Service not supported */
			break;
